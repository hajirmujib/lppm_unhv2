...
import 'package:lppm_unhv2/services/komentarPenelitianS.dart';
import 'package:lppm_unhv2/services/komentarProposalS.dart';
import 'package:sizer/sizer.dart';
import 'package:lppm_unhv2/widget/myColor.dart';
import 'package:shared_preferences/shared_preferences.dart';

class KomentarPenelitianC extends GetxController {
  var isLoading = true.obs;
  var komentarList = List<KomentarPenelitian>.empty().obs;
  ...
  @override
  void onInit() {
    // fetchKomentarPenelitian();
    super.onInit();
  }

  Future<void> fetchKomentarPenelitian() async {
    isLoading(true);
    try {
      var res = await KomentarServices()
          .getKomentarPenelitian(idKegiatan: idProposal.value);
      if (res != null) {
  ...
  Future<void> uploadKomentar() async {
    ...
    if (form.validate()) {
      ...
      try {
        List res = await KomentarPenelitianServices.addKomentar(
            idPenelitian: this.idProposal.value,
            idUser: idU,
            isi: this.isi.value);
  ...
  Future editKomentar({String id}) async {
    ...
    if (form.validate()) {
      ...
      var idU = pref.getString("idUser");
      try {
        List res = await KomentarPenelitianServices.editKomentar(
            idKomentar: id,
            idPenelitian: this.idProposal.value,
            idUser: idU,
  ...
  Future<void> deleteKomentar({String id}) async {
    try {
      ...
      List res = await KomentarPenelitianServices.deleteKomentar(id: id);
